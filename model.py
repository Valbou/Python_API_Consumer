...
class Model(Api):
    ...
    def save(self, log=False):
        ...
        if self.id is 0:
            response = self.post_inst(self.item, payload=self._build_dictionary())
            if response:
    ...
    def update(self):
        ...
        return self.patch_inst(self.item, payload=self._build_dictionary())

    def delete(self):
        """" DELETE - Delete instance in the API """
        return self.delete_inst(self.item, payload={'id': self.id})
    ...
    def control(self, dictionary:dict):
        ...
        for k, _ in dictionary.items():
            if not getattr(self, k) == dictionary[k]:
                return False
            elif self._is_object((0,getattr(self, k))) and not getattr(self, k).id == dictionary[k]:
                return False
        return True
